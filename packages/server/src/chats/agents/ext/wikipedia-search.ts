...
export class WikipediaSearch {
  ...
  private async _fetchSearchResults(query: string): Promise<SearchResults> {
    ...
    // console.log(`${this.baseUrl}?${searchParams.toString()}`);
    // const response = await fetch(`${this.baseUrl}?${searchParams.toString()}`);
    const response = await this.instance.get(`?${searchParams.toString()}`);
    if (response.data) {
  ...
  private async _fetchPage(page: string, redirect: boolean): Promise<Page> {
    ...
    const response = await this.instance.get(`?${params.toString()}`);
    if (response.data) {
      const data: PageResult = response.data;
