...
def build(fasta_path, var_path, BLAST_db, out_path, title, threads,start_pos=0,end_pos=-1):
    '''
    ...
    if end_pos != -1:
        seq_raw = seq_raw[start_pos:end_pos]
...
def generate_context(SOI):
    ...
    for sub_clu in clu:
        ...
        rp_stop, rp_start, rp_risk = find_min_loc(seq, risk_arr, fp_start+min_amp_len-primer_length, fp_start+max_amp_len-1, rng)
...
def design_context_seq(config, exclude_d):
    ...
    tik = time()
    with multiprocessing.Pool(processes=n_cpu) as pool:
        batch = [(seq_raw, start, stop, risk_arr, max_amp_len, min_amp_len, var_arr, rand_int[i]) for i in range(N)] #start, stop,
...
def get_primer(all_plex_info, config):
    ...
    for plex_id, plex_info in all_plex_info.items():
        ...
        while len(rP)<=0 and count_right<20:
            ...
            count_right+=1
        if count_right>=20:
            rP, fail = rP_generator.get(basic.revcomp(rP_design), rP_prefix, l_r_flag='right', check_BLAST=check_BLAST, **rP_setting)
            while len(rP) == 0:
                print('Fail to generate rP, update setting: %s' % plex_id)
                # detect failure mode with the most cases
                fail_mode = max(fail, key=fail.get)
