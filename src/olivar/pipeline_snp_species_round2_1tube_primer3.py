...
from .design_primer3 import PrimerSetBadnessFast
...
def build(fasta_path, var_path, BLAST_db, out_path, title, threads,start_pos,end_pos):
    '''
...
def generate_context(SOI):
    ...
    all_risk = []
    clu_mun = 1
    # collecting context seq according to snp clustering result
    print('clu',clu)
    for sub_clu in clu:
        ...
        if len(sub_clu)==1 or (sub_clu[-1]-sub_clu[0]) < 2*PRIMER_DESIGN_LEN:  # If only one member, only need to design one primer
            continue
        ...
        left_lim = max(fp_stop+2*PRIMER_DESIGN_LEN, rp_start+3*PRIMER_DESIGN_LEN-max_amp_len+1) # ee cannot be less than fp_stop+1/2*(min_amp_len-2*primer-length)
        if left_lim > stop:
            print(f'cluster {clu_mun} needs only 1 amplicon ')
            continue
        fp_start, fp_stop, fp_risk = find_min_loc(risk_arr, left_lim, rp_stop-1, rng) # rp_stop cannot move
...
def optimize(all_plex_info, config, species_candidates):
    ...
    for i_tube in range(1, n_tube+1):
        ...
        for plex_id in curr_tube:
            ...
            #print('range: ',len(all_plex_info[plex_id]['optimize']))
            count = 0
