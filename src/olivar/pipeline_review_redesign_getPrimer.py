...
from . import basic
from . import design0  # import design
from .ncbi_tools import BLAST_batch_short, ns_simulation
from .design0 import PrimerSetBadnessFast
...
def generate_context(SOI):
    ...
    for sub_clu in clu:
        ...
        rp_stop, rp_start, rp_risk = find_min_loc(risk_arr, fp_start + min_amp_len - primer_length,
...
def design_context_seq(build_ref, exclude_d, pre_index, config):
    ...
    tik = time()
    with multiprocessing.Pool(processes=n_cpu) as pool:
        batch = [(start, stop, risk_arr, max_amp_len, min_amp_len, var_arr, rand_int[i]) for i in
                 range(N)]  # start, stop,
    ...
    return all_plex_info, risk_arr, gc_arr, comp_arr, hits_arr, var_arr, all_loss, seq_raw, len(
        all_plex_info) + pre_index
...
def get_primer(all_plex_info, config):
    ...
    fP_generator = design0.primer_generator(temperature_fP, salinity)
    rP_generator = design0.primer_generator(temperature_rP, salinity)
    n = 0
...
def panel_redesign_optimize(optimize_plex_info, redesign_plex_dict, config, species_candidates,
                            panel_save_path):  # all_plex_info,
    '''
    ...
    # get primer
    # redesigned config
    temperature_fP = 55  # config['temperature']
    temperature_rP = 55  # config['temperature']
    salinity = config['salinity']
    fP_adapter = config['fP_prefix']
    rP_adapter = config['rP_prefix']
    default_fP_setting = {
    ...
    for i, (key, value) in enumerate(redesign_plex_dict.items()):
        ...
        if value == 'f':
            # generate fP
            fP_design = optimize_plex_info['all_plex_info'][key]['fP_design']
            fP_setting = deepcopy(default_fP_setting)
            fP, fail = fP_generator.get(fP_design, fP_prefix, check_BLAST=check_BLAST, **fP_setting)
            while len(fP) == 0:
                print('Fail to generate fP, update setting: %s' % key)
                # detect failure mode with the most cases
                fail_mode = max(fail, key=fail.get)
                # update setting
                if fail_mode == 'SNP_fail' and fP_setting['check_SNP'] == True:
                ...
                # generate primers again
                fP, fail = fP_generator.get(fP_design, fP_prefix, check_BLAST=check_BLAST, **fP_setting)
            optimize_plex_info['all_plex_info'][key]['fP_candidate'] = fP
            optimize_plex_info['all_plex_info'][key]['fP_setting'] = fP_setting

        elif value == 'r':

            # generate rP
            rP_design = optimize_plex_info['all_plex_info'][key]['rP_design']  # sequence to generate rP candidates
            rP_setting = deepcopy(default_rP_setting)
            rP, fail = rP_generator.get(rP_design, rP_prefix, check_BLAST=check_BLAST, **rP_setting)
            while len(rP) == 0:
                print('Fail to generate rP, update setting: %s' % key)
                # detect failure mode with the most cases
                fail_mode = max(fail, key=fail.get)
                # update setting
                if fail_mode == 'SNP_fail' and rP_setting['check_SNP'] == True:
                ...
                # generate primers again
                rP, fail = rP_generator.get(rP_design, rP_prefix, check_BLAST=check_BLAST, **rP_setting)
            optimize_plex_info['all_plex_info'][key]['rP_candidate'] = rP
            optimize_plex_info['all_plex_info'][key]['rP_setting'] = rP_setting
        ...
        # refresh plex optimize
        optimize = []
        plex_amp_len = []
        for i, fp in optimize_plex_info['all_plex_info'][key]['fP_candidate'].iterrows():
            for j, rp in optimize_plex_info['all_plex_info'][key]['rP_candidate'].iterrows():
                insert_len = optimize_plex_info['all_plex_info'][key]['insert_coords'][1] - \
                             optimize_plex_info['all_plex_info'][key]['insert_coords'][0] + 1
                # check amplicon length
                amp_len = fp['primer_len'] + fp['dist'] + insert_len + rp['dist'] + rp['primer_len']
                plex_amp_len.append(amp_len)
                optimize.append([fp, rp])
        optimize_plex_info['all_plex_info'][key]['optimize'] = optimize
    ...
    for i_tube in range(1, n_tube + 1):
        ...
        all_tube = [plex_id for plex_id, plex_info in optimize_plex_info['all_plex_info'].items() if
                    plex_info['tube'] == i_tube]
        ...
        for n, (plex_id, i) in enumerate(curr_index.items()):
            optimize_plex_info['all_plex_info'][plex_id]['fP'] = \
            optimize_plex_info['all_plex_info'][plex_id]['optimize'][i][0]
            optimize_plex_info['all_plex_info'][plex_id]['rP'] = \
            optimize_plex_info['all_plex_info'][plex_id]['optimize'][i][1]
...
def tiling(build_ref, exclude_dict, pre_amp_index, config):
    ...
    all_plex_info, risk_arr, gc_arr, comp_arr, hits_arr, var_arr, all_loss, seq_raw, current_index = design_context_seq(
        build_ref, exclude_dict, pre_amp_index, config)  # , exclude_dict
...
def validate(primer_pool, BLAST_db, out_path, title, max_amp_len, temperature, threads):
    ...
    # calculate dG
    gen = design0.primer_generator(temperature=temperature, salinity=0.18)
    ...
    df_val = pd.DataFrame({
        ...
        'self_align': [design0.WAlignScore(p) for p in seq_list],
        'dimer_score': all_bad,
        'BLAST_hits': all_hits
    })
