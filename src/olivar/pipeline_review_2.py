...
def build(config, merge_id, fasta, var_path, BLAST_db, out_path, title, threads):
    '''
    ...
    gc_func = partial(basic.get_GC_loss_2,config['min_GC'],config['max_GC']) #fix partial parameter
    atcg_func = partial(basic.get_ATGC_content_loss_2,config['min_atcg'],config['max_atcg'])
    with multiprocessing.Pool(processes=n_cpu) as pool:
        print('calculating GC content and sequence complexity...')
        all_gc = np.array(pool.map(gc_func, all_word))
        all_complexity = np.array(pool.map(atcg_func, all_word))
    #
    # all_gc = []
    # all_complexity = []
    # for word in all_word:
    #     all_gc.append(basic.get_GC_loss(config['min_GC'],config['max_GC'],word))
    #     all_complexity.append(basic.get_ATGC_content_loss_2(config['min_atcg'],config['max_atcg'], word))
    ...
    hits_arr = hits_arr/n_cycle  ## according to what kind of template to choose the constant to divide hits_arr
...
def generate_context(SOI):
    ...
    '''
    seq_raw, pre_index, config, a_start, a_stop, risk_arr, max_amp_len, min_amp_len, var_ar, merge_name, seed = SOI #start, stop,
    rng = default_rng(seed)

    #print(var_ar)
    #check_zero = np.array(var_ar)
    # if np.all(var_ar==0):
    if config['use_snp_cluster'] == False:
        clu = []
    ...
    default_fP_setting = {
        ...
        'max_GC': config['max_GC'], 
        #
        'min_atcg': config['min_atcg'],
        'max_atcg': config['max_atcg'],
        #
        #'min_complexity': config['min_complexity'], 
        'max_len': config['max_len'], 
        'check_SNP': False, # True
        'repeat_num': 4,
        'BLAST_db': None
    }
    default_rP_setting = deepcopy(default_fP_setting)
    primer_length = config['max_len']
    ############## caultion here if check_BLAST needddddddd!!!!!!!!
    ############## caultion here if check_BLAST needddddddd!!!!!!!!
    check_BLAST = False  
    ############## caultion here if check_BLAST needddddddd!!!!!!!!
    ...
    for sub_clu in clu:
        ...
        if config['use_snp_cluster'] == False: # no var condition
            start = sub_clu[0]
            stop = sub_clu[-1]
        ...
        ###### add
        fP_design = seq_raw[fp_start-1:fp_stop]

        print('Generating forward primer candidates...')
        fP_setting_tmp = deepcopy(default_fP_setting)
        fP, fail = fP_generator.get(fP_design, fP_adapter, check_BLAST=check_BLAST, **fP_setting_tmp)
        redesign_count = 0
        while len(fP) == 0:
            
            if redesign_count > 10:
                print('Fail to generate fP, update setting: %s' % plex_id)
               
                fail_mode = max(fail, key=fail.get)
                # update setting
                if fail_mode == 'SNP_fail' and fP_setting_tmp['check_SNP'] == True:
                    fP_setting_tmp['check_SNP'] = False
                elif fail_mode == 'GC_fail':
                    fP_setting_tmp['min_GC'] -= 0.05
                    fP_setting_tmp['max_GC'] += 0.05
                elif fail_mode == 'complexity_fail':
                    fP_setting_tmp['max_atcg'] += 0.05
                    #fP_setting_tmp['min_atcg'] -= 0.02
                elif fail_mode == 'repeat_fail':
                    fP_setting_tmp['repeat_num'] += 1
                    print(fP_setting_tmp['repeat_num'], plex_id, fP_design)
        ...
        ###### add 
        rP_design = basic.revcomp(seq_raw[rp_stop-1:rp_start])

        print('Generating reverse primer candidates...')
        rP_setting_tmp = deepcopy(default_rP_setting)
        rP, fail = rP_generator.get(rP_design, rP_adapter, check_BLAST=check_BLAST, **rP_setting_tmp)
        redesign_count = 0
        while len(rP) == 0:
            if redesign_count > 10:
                ...
                fail_mode = max(fail, key=fail.get)
                # update setting
                if fail_mode == 'SNP_fail' and rP_setting_tmp['check_SNP'] == True:
                    rP_setting_tmp['check_SNP'] = False
                elif fail_mode == 'GC_fail':
                    rP_setting_tmp['min_GC'] -= 0.05
                    rP_setting_tmp['max_GC'] += 0.05
                elif fail_mode == 'complexity_fail':
                    rP_setting_tmp['max_atcg'] += 0.05
                    #rP_setting_tmp['min_atcg'] -= 0.02
                elif fail_mode == 'repeat_fail':
                    rP_setting_tmp['repeat_num'] += 1
                    print(rP_setting_tmp['repeat_num'], plex_id, rP_design)
        ...
        plex_info = {
            #tube num
            'tube': plex_id%2 + 1,  #tube num
            #'tube': 1,
            'context_seq_coords': (fp_start, rp_start),
            'primers_coords': tuple((fp_start, fp_stop, rp_stop, rp_start)), 
            'insert_coords': (fp_stop+1, rp_stop-1), 
            'context_seq': seq_raw[fp_start-1:rp_start], 
            'context_seq_revcomp': basic.revcomp(seq_raw[fp_start-1:rp_start]), 
            'fP_design': fP_design, 
            'rP_design': rP_design, 
            'risk': tuple((fp_risk, rp_risk)),
            # add merge information
            'merge': merge_name,
            'fP_candidate': fP,
            'fP_setting': fP_setting_tmp,
            'rP_candidate': rP,
            'rP_setting': rP_setting_tmp
        }
        # plex info
        all_plex_info['amp_%d' % (plex_id)] = plex_info
        #caution plex_id ++
        ...
        left_lim = max(fp_stop+2*PRIMER_DESIGN_LEN, rp_start+3*PRIMER_DESIGN_LEN-max_amp_len+1) # ee不能小于fp_stop+1/2*(min_amp_len-2*primer-length)
        if left_lim > stop:
            print(f'cluster {clu_mun} needs only 1 amplicon ')
            continue
        fp_start, fp_stop, fp_risk = find_min_loc(risk_arr, left_lim, rp_stop-1, rng) # rp_stop 不能动
        ####### add    
        fP_design = seq_raw[fp_start-1:fp_stop]
        print('Generating forward primer candidates...')
        fP_setting_tmp = deepcopy(default_fP_setting)
        ...
        ####################################################
        # 
         # id of the next context sequence to be designed (0-based)
        #
        cs_id = 2
        while True:
            plex_id += 1
...
def design_context_seq(build_ref, exclude_d, pre_index, config):
    ...
    # if max(gc_arr) != 0:                                       
    #     gc_arr = gc_arr/max(gc_arr) # normalize

    # if max(var_arr) != 0:                                      
    #     var_arr = var_arr/max(var_arr) # normalize

    # make arrays the same length as seq_raw
    gc_arr = w_gc*np.concatenate((np.zeros(start-1), gc_arr, np.zeros(len(seq_raw)-stop)))
    comp_arr = w_complexity*np.concatenate((np.zeros(start-1), comp_arr, np.zeros(len(seq_raw)-stop)))
    ...
    var_arr = w_var*np.concatenate((np.zeros(start-1), var_arr, np.zeros(len(seq_raw)-stop)))
    ...
    #
    batch2 = (seq_raw, pre_index, config, start, stop, risk_arr, max_amp_len, min_amp_len, var_arr, merge_name, seed)
    all_plex_info, cur_index = generate_context(batch2)
    ...
    # all_context_seq, all_risk, loss = design2
    ...
    #assert cur_index==len(all_plex_info)+pre_index
    return all_plex_info, len(all_plex_info)+pre_index
...
def get_primer(all_plex_info, config):
    ...
    for plex_id, plex_info in all_plex_info.items():
        ...
        while len(fP) == 0:
            ...
            # if fail['complexity_fail']>0:
            #     fail_mode='complexity_fail'
            # else:
            fail_mode = max(fail, key=fail.get)
            # update setting
            if fail_mode == 'SNP_fail' and fP_setting['check_SNP'] == True:
                ...
                fP_setting['max_atcg'] += 0.05
                fP_setting['min_atcg'] -= 0.02
            elif fail_mode == 'repeat_fail':
                fP_setting['repeat_num'] += 1
                print(fP_setting['repeat_num'], plex_id, fP_design)
            # generate primers again
            fP, fail = fP_generator.get(fP_design, fP_prefix, check_BLAST=check_BLAST, **fP_setting)
        ...
        while len(rP) == 0:
            ...
            if fail_mode == 'SNP_fail' and rP_setting['check_SNP'] == True:
                ...
                rP_setting['max_atcg'] += 0.05
            elif fail_mode == 'repeat_fail':
                rP_setting['repeat_num'] += 1
                print(rP_setting['repeat_num'], plex_id, rP_design)
            # generate primers again
            rP, fail = rP_generator.get(rP_design, rP_prefix, check_BLAST=check_BLAST, **rP_setting)
...
def panel_optimize(all_plex_info, config, panel_save_path): # species_candidates,
    '''
    ...
    InitSATemp = 1000 + pool_size  # 10
    NUMSTEPS = 10 + int(pool_size/500)  # 10
    ZEROSTEPS = NUMSTEPS
    TimePerStep = 1000
...
def panel_save(all_plex_info, config,  panel_save_path): # species_candidates,
    '''
    save to csv
    Input:
...
def tiling(build_ref, exclude_dict, pre_amp_index, config):
    ...
    all_plex_info_primer, current_index = design_context_seq(build_ref, exclude_dict, pre_amp_index, config)
