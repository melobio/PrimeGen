...
# bottom CHOICE_RATE lowest risk primer design regions are choosen from candidate region
CHOICE_RATE = 0.3  #dt1 0.4
...
#     score2 = []
#     loc2 = np.array(range(3, 1867))
#     for i in loc2:
#         score2.append(sum(risk_arr[i - 1: i - 1 + PRIMER_DESIGN_LEN]))
    ...
    if len(loc) == 1:
        ...
        idx_rand = random.choice(idx_k_lowest)  #rng.choice(idx_k_lowest)   #      random.choice(idx_k_lowest)
    else:
        idx_rand = random.choice(range(len(loc))) #rng.choice(range(len(loc)))  #       random.choice(range(len(loc)))
...
def generate_context(SOI):
    ...
    for sub_clu in clu:
        ...
        while True:
            ...
            fp_start, fp_stop, fp_risk = find_min_loc(risk_arr, left_lim, rp_stop-1, rng)
#             left_lim = max(all_context_seq[cs_id-2][3]+1, rp_start+3*PRIMER_DESIGN_LEN-max_amp_len+1)
#             fp_start, fp_stop, fp_risk = find_min_loc(risk_arr, left_lim, rp_stop+PRIMER_DESIGN_LEN, rng)
            # rp
            left_lim = max(fp_start+min_amp_len-PRIMER_DESIGN_LEN, rp_start+PRIMER_DESIGN_LEN+1)
            right_lim = fp_start+max_amp_len-1
            if right_lim > stop:
...
#     print('total amplicons: %d' % (i+1))
    ...
    print('all_context_seq',all_context_seq)
#     if len(all_context_seq)==0:
#         cover_start = 0
#         cover_stop = len(seq_raw)
#     else:
...
def optimize(all_plex_info, config, species_candidates):
    ...
    for i_tube in range(1, n_tube+1):
        ...
        # plot learning curve
        fig = plt.figure(figsize=(8, 5))
        df_loss = pd.DataFrame({'loss':learning_curve})
        df_loss.to_csv('./optimization_loss.csv')
...
def tiling(ref_path, out_path, title, max_amp_len, min_amp_len, w_egc, w_lc, w_ns, w_var, temperature, 
    salinity, dG_max, min_GC, max_GC, min_complexity, max_len, fP_prefix, rP_prefix, seed, threads, species_primer_candidates, exclude_dict):
    '''
    ...
    all_plex_info, risk_arr, gc_arr, comp_arr, hits_arr, var_arr, all_loss, seq_raw = design_context_seq(config, exclude_dict)
