...
metadata = {
    'protocolName': 'MGIEasy_RNA方向性_RNA富集_1_1',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    deep_plate = protocol.load_labware('nest_96_wellplate_2ml_deep', '6')
    # Pipettes

    right_pipette = protocol.load_instrument('p300_multi_gen2', mount='right', tip_racks=[tips200_1,tips200_2,tips200_3,tips200_4,tips200_5,tips200_6])
    ...
    for well_name in use_plate_well:
        temp_plate.wells_by_name()[well_name].load_liquid(liquid=RNA_sample, volume=25)
    ...
    transfer_all(right_pipette, deep_plate,temp_plate,  transfer_info_right1,mix_flag=1,mix_value=[10,30])
    protocol.delay(minutes=5)
    temp_shaker.deactivate()
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1),'3', 50))
    ...
    for ii in range(transfer_times):
        transfer_info_right3.append(('2',str(ii + 1),  50))
    transfer_all(right_pipette, deep_plate,temp_plate, transfer_info_right3,mix_flag=1,mix_value=[10,30])
    mag_module.engage(height_from_base=5)
    protocol.delay(minutes=2)

    transfer_info_right4 = []
    for ii in range(transfer_times):
        transfer_info_right4.append((str(ii + 1),'4', 50))
    transfer_all(right_pipette, temp_plate,deep_plate, transfer_info_right4)

    transfer_info_right5 = []
    for ii in range(transfer_times):
        transfer_info_right5.append(('2', str(ii + 1), 50))
    transfer_all(right_pipette, deep_plate, temp_plate, transfer_info_right5)
    protocol.delay(minutes=5)

    transfer_info_right6 = []
    for ii in range(transfer_times):
        transfer_info_right6.append((str(ii + 1),'4', 50))
    transfer_all(right_pipette, temp_plate,deep_plate, transfer_info_right6)
