...
def run(protocol: protocol_api.ProtocolContext):
    ...
    UDB_Primer_MIX = protocol.define_liquid(
        name="UDB_Primer_MIX",
        description="UDB_Primer_MIX",
        display_color="#00FF00",
    )
    # 加载液体
    ...
    if use_col_nums > 6:
        for well in temp_plate.columns_by_name()['1']:
            temp_plate.wells_by_name()[well.display_name].load_liquid(liquid=PCR_MIX, volume=163)
        liquid_list = calculate_liquid(Sample_nums - 48, MIX_volume - 163*8)
        for ii in range(8):
    ...
    for well_name in use_plate_well:
        ...
        plate1.wells_by_name()[well_name].load_liquid(liquid=UDB_Primer_MIX, volume=30)
    # 执行指令
    ...
    for ii in range(transfer_times):
        transfer_info_left1.append((str(ii + 1), str(ii + 1), 4))
    ...
    steps2 = [{'temperature': 95, 'hold_time_seconds': 15}, {'temperature': 60, 'hold_time_seconds': 30},
    ...
    thermocycler_module.execute_profile(steps=steps2, repetitions=15, block_max_volume=50)
