...
##
## This code block is suitable for post-hybridization PCR in protocols such as exome digestion/exome digestion assisted protocols.
##
metadata = {
...
def transfer_all(pipette, source_plate, dest_plate, transfer_info,mix_flag = 0,mix_value =[]):
    ...
    Args:
    - pipette: The pipette being used
    ...
    - mix_value: Mixing information as a list, [number of times to mix, volume]
    """
...
def calculate_liquid(sample_num, liquid_volume,cal_type=1):
    ...
    if sample_num < 8:
        ...
        if cal_type ==1:
            ...
            count1 = int(sample_num / 8)
            count2 = sample_num % 8
            per_sample_liquid = liquid_volume / (8 * (count1 + 0.25) + count2)

            for ii in range(8):
                if count2 > 0:
                    liquid_list.append((count1 + 1.25) * per_sample_liquid)
                    count2 = count2 - 1
                else:
                    liquid_list.append((count1 + 0.25) * per_sample_liquid)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    elif use_col_nums>=3 and use_col_nums<6:
        half_volume_1 = (MIX_volume/(Sample_nums+8))*3.25
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=PCR_MIX, volume=half_volume_1)
        liquid_list = calculate_liquid(Sample_nums-24,MIX_volume-half_volume_1*8,cal_type=4)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[8:16][ii]].load_liquid(liquid=PCR_MIX, volume=liquid_list[ii])

    elif use_col_nums>=6 and use_col_nums<9:
        half_volume_1 = (MIX_volume/(Sample_nums+8))*3.25
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=PCR_MIX, volume=half_volume_1)
            temp_plate.wells_by_name()[all_plate_well[8:16][ii]].load_liquid(liquid=PCR_MIX, volume=half_volume_1)
        liquid_list = calculate_liquid(Sample_nums-48,MIX_volume-half_volume_1*16,cal_type=4)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[16:24][ii]].load_liquid(liquid=PCR_MIX, volume=liquid_list[ii])
    else:
        half_volume_1 = (MIX_volume/(Sample_nums+8))*3.25
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=PCR_MIX, volume=half_volume_1)
            temp_plate.wells_by_name()[all_plate_well[8:16][ii]].load_liquid(liquid=PCR_MIX, volume=half_volume_1)
            temp_plate.wells_by_name()[all_plate_well[16:24][ii]].load_liquid(liquid=PCR_MIX, volume=half_volume_1)
        liquid_list = calculate_liquid(Sample_nums-72,MIX_volume-half_volume_1*24,cal_type=4)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[24:32][ii]].load_liquid(liquid=PCR_MIX, volume=liquid_list[ii])
