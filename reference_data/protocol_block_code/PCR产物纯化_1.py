...
##
## This code block is suitable for single-cell chromosome copy number variation, chromosome copy number variation, cell-free DNA,
## exome enzymatic fragmentation, general DNA, enzymatic DNA, Fast RNA, MGIEasy RNA directional, ATOPlexHIV multiplex amplification product purification,
## ATOPlex MPXV, MGIEasy_fast, exome enzymatic_POST-PCR product purification_9/exome enzymatic auxiliary_PCR product purification_2 
## and other protocols for PCR amplification product purification.
##
metadata = {
    'protocolName': 'MGIEasy_fastRNA_PCR产物纯化_6',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1),'5',beads_transfer_volume+Single_sample_volume))
    ...
    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right7,mix_flag=1,mix_value=[10,30])
    protocol.delay(minutes=5)
    mag_module.engage(height_from_base=5)
    protocol.delay(minutes=5)
    # Transfer
