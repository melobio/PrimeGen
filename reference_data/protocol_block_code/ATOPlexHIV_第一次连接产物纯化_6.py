...
#
## This code block is suitable for ATOPlexHIV_First Connection Product Purification_6/ATOPlexMPVX_First Connection Product Purification_6.
#
metadata = {
    'protocolName': 'ATOPlexHIV-6',
    'author': 'MGIX',
...
def transfer_all(pipette, source_plate, dest_plate, transfer_info,mix_flag = 0,mix_value =[]):
    ...
    - transfer_info: List containing transfer information, each element is a tuple (source column, destination column, volume)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Beads_volume = input_DNA_Beads_volume_1
    TE_Buffer_volume = input_TE_Buffer_volume_1
    Ethanol_80_volume = input_Ethanol_80_volume_1
    use_col_nums = int(Sample_nums/8)
    if Sample_nums%8 !=0:
        use_col_nums = use_col_nums+1
    # Load 200ul filter tips rack
    ...
    tips200_8 = protocol.load_labware('oopentrons_96_tiprack_300ul', '11')
    # Load magnetic module and PCR plate on the magnetic module
    ...
    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right2, mix_flag=1, mix_value=[10, 25])
    # Pause for 5 minutes
    ...
    transfer_all(right_pipette, mag_plate, reservoir_plate, transfer_info_right3)

    transfer_info_right4 = []
    for ii in range(transfer_times):
        transfer_info_right4.append(('3', str(ii + 1), 160))

    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right4)
    ...
    transfer_all(right_pipette, mag_plate, reservoir_plate, transfer_info_right5)

    transfer_info_right6 = []
    for ii in range(transfer_times):
        transfer_info_right6.append(('4', str(ii + 1), 160))

    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right6)
    # Pause for 30 seconds
    protocol.delay(seconds=30)

    transfer_info_right7 = []
    for ii in range(transfer_times):
        transfer_info_right7.append((str(ii + 1), '7', 160))

    transfer_all(right_pipette, mag_plate, reservoir_plate, transfer_info_right7)
    protocol.delay(minutes=5)
    ...
    for ii in range(transfer_times):
        transfer_info_right8.append(('2', str(ii + 1), 20))

    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right8,mix_flag=1,mix_value=[10,10])
