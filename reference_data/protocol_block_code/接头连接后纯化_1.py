...
##
## This code block is suitable for purifying adapter ligation products in protocols such as Single Cell Chromosome Copy Number Variation, 
## Chromosome Copy Number Variation, Cell-free DNA, Exome Enzyme Digestion, General DNA, Enzyme DNA Digestion,
## MGIEasy_fast, MGIEasy_fastRNA, MGIEasy_RNA Directional, MGIEasy_RNA Directional_Second Strand Product Purification_4.py, etc.
##
metadata = {
    'protocolName': 'Cell-free DNA-3',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Beads_volume = input_DNA_Clean_Beads_volume
    TE_Buffer_volume = input_TE_Buffer_volume
    Ethanol_80_volume = input_Ethanol_80_volume
    final_transfer_volume = input_final_transfer_volume
    ...
    for well_name in use_plate_well:
        mag_plate.wells_by_name()[well_name].load_liquid(liquid=connect_products, volume=Single_sample_volume)
    ...
    for ii in range(transfer_times):
        transfer_info_right1.append(('1', str(ii + 1), Beads_transfer_volume))
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1), '5', Single_sample_volume+Beads_transfer_volume))

    transfer_all(right_pipette, mag_plate, reservoir_plate, transfer_info_right2)

    transfer_info_right3 = []
    for ii in range(transfer_times):
        transfer_info_right3.append(('3', str(ii + 1), Ethanol_80_transfer_volume))
    ...
    for ii in range(transfer_times):
        transfer_info_right4.append((str(ii + 1), '6', Ethanol_80_transfer_volume))

    transfer_all(right_pipette, mag_plate, reservoir_plate, transfer_info_right4)

    transfer_info_right5 = []
    for ii in range(transfer_times):
        transfer_info_right5.append(('4', str(ii + 1), Ethanol_80_transfer_volume))
    ...
    for ii in range(transfer_times):
        transfer_info_right6.append((str(ii + 1), '7', Ethanol_80_transfer_volume))
    ...
    for ii in range(transfer_times):
        transfer_info_right7.append(('2', str(ii + 1), TE_Buffer_transfer_volume))
    ...
    if final_transfer_volume<20:
        for ii in range(transfer_times):
            transfer_info_8.append((str(ii + 1), str(ii + 1), final_transfer_volume))

        transfer_all(left_pipette, mag_plate, plate1, transfer_info_8)    
    else:
    
        for ii in range(transfer_times):
            transfer_info_8.append((str(ii + 1), str(ii + 1), final_transfer_volume))

        transfer_all(right_pipette, mag_plate,plate1, transfer_info_8)
