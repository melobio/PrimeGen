...
#
## This code block is for ATOPlexHIV_Second_connection_product_purification_7/ATOPlexMPVX_Second_connection_product_purification_7.
#
metadata = {
    'protocolName': 'ATOPlexHIV-7',
    'author': 'MGIX',
...
def transfer_all(pipette, source_plate, dest_plate, transfer_info, mix_flag=0, mix_value=[]):
    """
    A generic function for transferring samples.
    Args:
    - pipette: The pipette used
    - source_plate: Source plate
    - dest_plate: Destination plate
    - transfer_info: A list containing transfer information, where each element is a tuple (source column, destination column, volume)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Beads_volume = input_DNA_Beads_volume_2
    TE_Buffer_volume = input_TE_Buffer_volume_2
    Ethanol_80_volume = input_Ethanol_80_volume_2
    ...
    for well_name in use_plate_well:
        mag_plate.wells_by_name()[well_name].load_liquid(liquid=connect_products, volume=20)
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1), '5', 40))
    ...
    for ii in range(transfer_times):
        transfer_info_right7.append(('2', str(ii + 1), 27))

    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right7, mix_flag=1, mix_value=[10, 10])
    protocol.delay(minutes=5)

    # Liquid transfer
    transfer_info_right8 = []
    for ii in range(transfer_times):
        transfer_info_right8.append((str(ii + 1), str(ii + 1), 25))
