...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Single_Enzyme_digestion_volume = input_Single_sample_volume
    ...
    UDB_adapter = protocol.define_liquid(
        name="UDB_adapter",
    ...
    if use_col_nums>6:
        for ii in range(8):
            ...
            half_volume_2 = (TE_buffer_volume/(Sample_nums+8))*6.5
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=Adapter_ligation_solution, volume=half_volume_1)
            temp_plate.wells_by_name()[all_plate_well[16:24][ii]].load_liquid(liquid=TE_buffer, volume=half_volume_2)
        liquid_list1 = calculate_liquid(Sample_nums-48,Adapter_ligation_volume-half_volume_1*8,cal_type=2)
        liquid_list2 = calculate_liquid(Sample_nums-48,TE_buffer_volume-half_volume_2*8,cal_type=2)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[8:16][ii]].load_liquid(liquid=Adapter_ligation_solution, volume=liquid_list1[ii])
            temp_plate.wells_by_name()[all_plate_well[24:32][ii]].load_liquid(liquid=TE_buffer, volume=liquid_list2[ii])
    else:
        liquid_list1 = calculate_liquid(Sample_nums,Adapter_ligation_volume)
        liquid_list2 = calculate_liquid(Sample_nums,TE_buffer_volume)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=Adapter_ligation_solution, volume=liquid_list1[ii])
