...
metadata = {
    'protocolName': 'MGIEasy ATOPlexHIV-5',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Adapter_ligation_volume = input_Adapter_ligation_volume_5
    ...
    tips200_2 = protocol.load_labware('oopentrons_96_tiprack_300ul', '4')
    tips200_3 = protocol.load_labware('opentrons_96_tiprack_300ul', '5')

    plate1 = protocol.load_labware('biorad_96_wellplate_200ul_pcr', '1')
    # Load temperature module and PCR plate on temperature module
    ...
    if use_col_nums>6:
        for well in temp_plate.columns_by_name()['1']:
            temp_plate.wells_by_name()[well.display_name].load_liquid(liquid=Adapter_ligation_solution, volume=195)
        liquid_list = calculate_liquid(Sample_nums,Adapter_ligation_volume-195)
    ...
    transfer_info_left1 = []
    for ii in range(transfer_times):
        transfer_info_left1.append((str(ii + 1), str(ii + 1), 3))
