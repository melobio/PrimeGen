...
metadata = {
    'protocolName': 'MGIEasy_RNA方向性_二链产物纯化_2',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    for well_name in use_plate_well:
        mag_plate.wells_by_name()[well_name].load_liquid(liquid=connect_products, volume=50)
    ...
    for ii in range(transfer_times):
        transfer_info_right1.append(('1',str(ii + 1), 75))

    transfer_all(right_pipette,reservoir_plate,mag_plate,transfer_info_right1,mix_flag=1,mix_value=[10,100])
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1),'5',125))
    ...
    for ii in range(transfer_times):
        transfer_info_right7.append(('2', str(ii + 1), 42))

    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right7,mix_flag=1,mix_value=[10,23])
    protocol.delay(minutes=5)
    mag_module.engage(height_from_base=5)
    protocol.delay(minutes=5)
    # 移液
    ...
    for ii in range(transfer_times):
        transfer_info_right8.append((str(ii + 1), str(ii + 1), 40))
