...
metadata = {
    'protocolName': 'MGIEasy ATOPlexHIV-3',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Enzyme_MIX_volume = input_Enzyme_MIX_volume_3
    ...
    liquid_list = calculate_liquid(Sample_nums,Enzyme_MIX_volume)
    for ii in range(8):
...
#         temp_plate.wells_by_name()[well.display_name].load_liquid(liquid=Enzyme_MIX, volume=195)   
    use_plate_well = all_plate_well[:Sample_nums]
    for well_name in use_plate_well: 
        Sample_plate.wells_by_name()[well_name].load_liquid(liquid=sample, volume=50) 
    ...
    steps1 = [{'temperature': 4, 'hold_time_minutes': 1}, {'temperature': 30, 'hold_time_minutes': 12},
              {'temperature': 65, 'hold_time_minutes': 20}, {'temperature': 4, 'hold_time_minutes': 1}]
