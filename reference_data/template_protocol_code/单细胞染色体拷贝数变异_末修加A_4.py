...
metadata = {
    'protocolName': '单细胞染色体拷贝数变异_末修加A_4',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    MIX_volume = input_A_MIX_volume_4
    ...
    Repair_MIX = protocol.define_liquid(
        name="Repair_MIX",
        description="Repair_MIX",
        display_color="#00FF00",
    )
    # 加载液体
    ...
    for ii in range(8):
        ...
        temp_plate.wells_by_name()[well_name].load_liquid(liquid=Repair_MIX, volume=liquid_list[ii])
    ...
    for well_name in use_plate_well:
        plate1.wells_by_name()[well_name].load_liquid(liquid=sample, volume=41)
    # 执行指令
    ...
    steps1 = [{'temperature': 37, 'hold_time_minutes': 10}, {'temperature': 65, 'hold_time_minutes': 15}
