...
metadata = {
    'protocolName': '单细胞染色体拷贝数变异_PCR扩增_7',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    MIX_volume = input_PCR_MIX_volume_7
    ...
    if use_col_nums > 6:
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=PCR_MIX, volume=188.5)
        liquid_list = calculate_liquid(Sample_nums - 48, MIX_volume - 188.5)
        for ii in range(8):
    ...
    for ii in range(transfer_times):
        if ii < 6:
            transfer_info_right1.append(('1', str(ii + 1), 29))
        else:
            transfer_info_right1.append(('2', str(ii + 1), 29))
    ...
    steps1 = [{'temperature': 98, 'hold_time_minutes': 2}]
    steps2 = [{'temperature': 98, 'hold_time_seconds': 15}, {'temperature': 56, 'hold_time_seconds': 15},
              {'temperature': 72, 'hold_time_seconds': 30}]
    steps3 = [{'temperature': 72, 'hold_time_minutes': 5}, {'temperature': 4, 'hold_time_minutes': 1}]
    thermocycler_module.execute_profile(steps=steps1, repetitions=1, block_max_volume=50)
    thermocycler_module.execute_profile(steps=steps2, repetitions=12, block_max_volume=50)
