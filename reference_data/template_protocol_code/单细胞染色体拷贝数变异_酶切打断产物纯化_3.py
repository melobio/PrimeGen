...
metadata = {
    'protocolName': '单细胞染色体拷贝数变异_酶切打断产物纯化_3',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Fragment_products = protocol.define_liquid(
        name="Fragment_products",
        description="Fragment_products",
    ...
    for well_name in use_plate_well:
        mag_plate.wells_by_name()[well_name].load_liquid(liquid=Fragment_products, volume=50)
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1), '5', 100))
    ...
    for ii in range(transfer_times):
        transfer_info_right7.append(('2', str(ii + 1), 43))
