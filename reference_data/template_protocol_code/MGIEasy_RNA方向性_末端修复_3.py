...
metadata = {
    'protocolName': 'MGIEasy_RNA方向性_末端修复_3',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    ERAT_MIX = protocol.define_liquid(
        name="ERAT_MIX",
        description="ERAT_MIX",
        display_color="#00FF00",
    )
    # Load liquids
    all_plate_well = [f"{chr(65 + i)}{j}" for j in range(1, 12 + 1) for i in range(8)]
    liquid_list = calculate_liquid(Sample_nums, ERAT_MIX_volume)
    for ii in range(8):
        temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=ERAT_MIX, volume=liquid_list[ii])

    use_plate_well = all_plate_well[:Sample_nums]
    for well_name in use_plate_well:
        thermocycler_plate.wells_by_name()[well_name].load_liquid(liquid=sample, volume=40)
