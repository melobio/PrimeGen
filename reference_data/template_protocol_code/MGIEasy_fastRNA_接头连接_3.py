...
metadata = {
    'protocolName': 'MGIEasy_fastRNA_接头连接_3',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    ligation_mix_volume = input_ligation_MIX_volume_3
    ...
    if use_col_nums > 6:
        for well in temp_plate.columns_by_name()['1']:
            temp_plate.wells_by_name()[well.display_name].load_liquid(liquid=ligation_mix, volume=163)
        liquid_list = calculate_liquid(Sample_nums - 48, ligation_mix_volume - 163)
        for ii in range(8, 16):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=ligation_mix,volume=liquid_list[ii])
    else:
        liquid_list = calculate_liquid(Sample_nums, ligation_mix_volume)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=ligation_mix,volume=liquid_list[ii])
    ...
    thermocycler_module.set_lid_temperature(37)
    thermocycler_module.execute_profile(steps=[
        {'temperature': 23, 'hold_time_minutes': 15},
