...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if use_col_nums > 6:
        for well in temp_plate.columns_by_name()['1']:
            temp_plate.wells_by_name()[well.display_name].load_liquid(liquid=PCR_MIX, volume=175.5)
        liquid_list = calculate_liquid(Sample_nums - 48, MIX_volume - 175.5)
        for ii in range(8):
    ...
    for well_name in use_plate_well:
        thermocycler_plate.wells_by_name()[well_name].load_liquid(liquid=sample, volume=23)
    ...
    for ii in range(transfer_times):
        if ii < 6:
            transfer_info_right1.append(('1', str(ii + 1), 27))
        else:
            transfer_info_right1.append(('2', str(ii + 1), 27))
