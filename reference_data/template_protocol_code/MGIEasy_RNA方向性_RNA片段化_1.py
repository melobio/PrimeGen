...
metadata = {
    'protocolName': 'MGIEasy_RNA方向性_RNA片段化_1',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    RT_MIX_volume = input_RT_MIX_volume_1
    second_strand_mix_MIX_volume =input_second_strand_MIX_volume_1
    Fragmentation_Buffer_volume = input_Fragmentation_Buffer_volume_1
    ...
    if use_col_nums > 6:
        ...
        liquid_list_3 = calculate_liquid(Sample_nums - 48, second_strand_mix_MIX_volume - 195)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[24:32][ii]].load_liquid(liquid=second_strand_mix, volume=liquid_list_3[ii])
    else:
        liquid_list_3 = calculate_liquid(Sample_nums, second_strand_mix_MIX_volume)
        for ii in range(8):
    ...
    for ii in range(transfer_times):
        transfer_info_left1.append(('1', str(ii + 1), 4))
    ...
    steps1 = [{'temperature': 94, 'hold_time_seconds': 480}]
    ...
    transfer_info_left2 = []
    for ii in range(transfer_times):
        transfer_info_left2.append(('2', str(ii + 1), 6))
