...
metadata = {
    'protocolName': '单细胞染色体拷贝数变异_接头连接_5',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    ligation_mix_volume = input_ligation_mix_volume_5
    ...
    if use_col_nums > 6:
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[ii]].load_liquid(liquid=ligation_mix, volume=162.5)
        liquid_list2 = calculate_liquid(Sample_nums - 48, ligation_mix_volume - 162.5)
        for ii in range(8):
            temp_plate.wells_by_name()[all_plate_well[8:16][ii]].load_liquid(liquid=ligation_mix, volume=liquid_list2[ii])
    else:
    ...
    transfer_info_left1 = []
    for ii in range(transfer_times):
        transfer_info_left1.append((str(ii + 1), str(ii + 1), 15))
