...
metadata = {
    'protocolName': 'MGIEasy ATOPlexHIV-1',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    MIX_volume = input_PCR_MIX_volume_1
    ...
    if use_col_nums>6:
        ...
        liquid_list = calculate_liquid(Sample_nums-48,MIX_volume-195)
        for ii  in range(8):
    ...
    for well_name in use_plate_well: 
        Sample_plate.wells_by_name()[well_name].load_liquid(liquid=sample, volume=25)
    ...
    for ii in range(transfer_times):
        transfer_info_left1.append((str(ii + 1), str(ii + 1), 20))

    transfer_all(left_pipette, Sample_plate, thermocycler_plate, transfer_info_left1)
    ...
    transfer_all(right_pipette, temp_plate, thermocycler_plate, transfer_info_right1,mix_flag=1,mix_value=[10,20])

    thermocycler_module.close_lid()
    thermocycler_module.set_lid_temperature(105)
    steps1 = [{'temperature': 50, 'hold_time_minutes': 30}, {'temperature': 94, 'hold_time_minutes': 3}]
    steps2 = [{'temperature': 94, 'hold_time_seconds': 30}, {'temperature': 58, 'hold_time_seconds': 45},
           {'temperature': 72, 'hold_time_minutes': 2}]
    
    steps3 = [{'temperature': 72, 'hold_time_minutes': 5}, {'temperature': 12, 'hold_time_minutes': 1}]
    thermocycler_module.execute_profile(steps=steps1, repetitions=1,block_max_volume=50)
    thermocycler_module.execute_profile(steps=steps2, repetitions=42,block_max_volume=50)
    thermocycler_module.execute_profile(steps=steps3, repetitions=1,block_max_volume=50)
    thermocycler_module.set_block_temperature(12, hold_time_seconds=None)
