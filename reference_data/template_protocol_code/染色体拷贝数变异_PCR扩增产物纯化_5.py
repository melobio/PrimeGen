...
metadata = {
    'protocolName': '染色体拷贝数变异_PCR扩增产物纯化_6',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    Beads_volume = input_DNA_Beads_volume_5
    TE_Buffer_volume = input_TE_Buffer_volume_5
    Ethanol_80_volume = input_Ethanol_80_volume_5
    ...
    for ii in range(transfer_times):
        transfer_info_right1.append((str(ii + 1),'5',  100))

    transfer_all(right_pipette, mag_plate, reservoir_plate,transfer_info_right1)

    transfer_info_right2 = []
    for ii in range(transfer_times):
        transfer_info_right2.append(('3', str(ii + 1),200))

    transfer_all(right_pipette, reservoir_plate, mag_plate,transfer_info_right2)
    protocol.delay(seconds=30)

    transfer_info_right3 = []
    for ii in range(transfer_times):
        transfer_info_right3.append((str(ii + 1), '6', 200))

    transfer_all(right_pipette, mag_plate, reservoir_plate, transfer_info_right3)

    transfer_info_right4 = []
    for ii in range(transfer_times):
        transfer_info_right4.append(('4', str(ii + 1), 200))
    ...
    for ii in range(transfer_times):
        transfer_info_right5.append((str(ii + 1), '7', 200))
    ...
    for ii in range(transfer_times):
        transfer_info_right6.append(('2', str(ii + 1), 25))

    transfer_all(right_pipette, reservoir_plate, mag_plate, transfer_info_right6,mix_flag=1,mix_value=[10,20])
    protocol.delay(minutes=5)
    mag_module.engage(height_from_base=5)
    protocol.delay(minutes=5)
    # 移液
    ...
    for ii in range(transfer_times):
        transfer_info_right7.append((str(ii + 1), str(ii + 1), 23))

    transfer_all(right_pipette, mag_plate,plate1, transfer_info_right7)
