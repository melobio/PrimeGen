...
metadata = {
    'protocolName': 'MGIEasy PCRfree-酶切打断后磁珠双选_2',
    'author': 'MGIX',
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    liquid_list = calculate_liquid(Sample_nums,Beads_volume)
    for ii in range(8):
...
#         deep_plate.wells_by_name()[well.display_name].load_liquid(liquid=En_Breads, volume=Beads_volume/8)
    liquid_list = calculate_liquid(Sample_nums,TE_Buffer_volume)
    for ii in range(8):
...
#         deep_plate.wells_by_name()[well.display_name].load_liquid(liquid=En_TE, volume=TE_Buffer_volume/8)
    ...
    for ii in range(transfer_times):
        transfer_info_right1.append(('1',str(ii+1),32))
    ...
    for ii in range(transfer_times):
        transfer_info_right2.append((str(ii + 1), str(ii + 1), 92))
    ...
    transfer_info_left1 = []
    for ii in range(transfer_times):
        transfer_info_left1.append(('1', str(ii + 1), 16))
    ...
    for ii in range(transfer_times):
        transfer_info_right4.append(('2',str(ii + 1), 47))
